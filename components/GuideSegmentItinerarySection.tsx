        ...
        {segments.map((segment) => {
          ...
          return (
            <article id={`day-${segment.dayStart}`} key={segment.id} className="scroll-mt-24 py-10 md:py-14">
              ...
              {segment.verifiedRoutes.map((route) => <section key={route.title} className="mt-7 border-y border-white/10 py-4"><div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.22em] text-white/45"><Navigation className="h-3.5 w-3.5" />{route.title}</div>{route.summary ? <p className="mt-3 max-w-3xl text-sm leading-7 text-white/75">{route.summary}</p> : null}{route.linkedSpots?.length ? <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{route.linkedSpots.map((spot) => <article key={spot} className="overflow-hidden border border-white/10 bg-white/[0.025]"><div className="relative aspect-[4/3] bg-black/20"><FallbackImage src={undefined} alt={`${spot} 景点照片`} fill sizes="(max-width: 640px) 100vw, 33vw" className="object-cover" /></div><div className="p-4"><p className="text-base font-medium leading-6 text-white">{spot}</p></div></article>)}</div> : null}</section>)}
