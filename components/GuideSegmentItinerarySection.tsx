import { BedDouble, MapPin, Navigation, Route } from 'lucide-react'
  ...
  return (
    <section aria-labelledby="itinerary-heading" className="min-w-0">
      <div className="border-b border-white/10 pb-5">
        ...
        <h2 id="itinerary-heading" className="mt-2 font-display text-4xl leading-none text-white md:text-5xl">城市行程</h2>
      </div>
      <div className="divide-y divide-white/10">
        {segments.map((segment) => {
          const video = segment.media?.map((item) => ({ label: item.label, id: youtubeId(item.url) })).find((item) => item.id)
          const spots = Array.from(new Set(segment.verifiedRoutes.flatMap((route) => route.linkedSpots || [])))
          ...
          return (
            <article id={`day-${segment.dayStart}`} key={segment.id} className="scroll-mt-24 py-10 md:py-14">
              ...
              <div className="mt-5 flex flex-wrap gap-2 text-xs text-white/68">
                {spots.length ? <span className="inline-flex items-center gap-1.5 border border-white/10 bg-white/[0.035] px-3 py-1.5"><MapPin className="h-3.5 w-3.5" />{spots.length} 个地点</span> : null}
              ...
              {spots.length ? <section aria-label={`${dayLabel} 相关地点`} className="mt-7"><p className="text-xs font-semibold uppercase tracking-[0.22em] text-white/48">相关地点</p><div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{spots.map((spot) => <article key={spot} className="overflow-hidden border border-white/10 bg-white/[0.025]"><div className="relative aspect-[4/3] bg-black/20"><FallbackImage src={undefined} alt={`${spot} 景点照片`} fill sizes="(max-width: 640px) 100vw, 33vw" className="object-cover" /></div><div className="p-4"><p className="text-base font-medium leading-6 text-white">{spot}</p></div></article>)}</div></section> : null}
              ...
              {segment.accommodation ? <section aria-label={`${dayLabel} 当日住宿`} className="mt-7 border border-sky-200/15 bg-sky-300/[0.055] p-4 md:p-5"><div className="flex items-center gap-2 text-sky-100/82"><BedDouble className="h-4 w-4" /><p className="text-xs font-semibold uppercase tracking-[0.22em]">当日住宿</p></div><div className="mt-4 grid gap-4 md:grid-cols-[180px_minmax(0,1fr)]"><div className="relative aspect-[4/3] overflow-hidden bg-black/20 md:aspect-[3/2]"><FallbackImage src={undefined} alt={`${segment.city} 住宿照片`} fill sizes="(max-width: 768px) 100vw, 180px" className="object-cover" /></div><div className="min-w-0"><p className="font-medium text-white">{segment.accommodation}</p>{segment.accommodationNote ? <p className="mt-2 text-sm leading-6 text-white/65">{segment.accommodationNote}</p> : null}</div></div></section> : null}
