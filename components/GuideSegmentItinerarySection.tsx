    ...
    <div className="divide-y divide-white/10">
      {segments.map((segment) => {
        ...
        return <article id={`day-${segment.dayStart}`} key={segment.id} className="scroll-mt-24 py-10 md:py-14">
          ...
          <p className="mt-5 max-w-[800px] text-[15px] leading-[1.85] text-white/76 md:text-base">{segment.summary}</p>
          <div className="mt-8 divide-y divide-white/10 border-y border-white/10">
            {routes.map((route) => {
              ...
              return <section key={dayNumber} id={`day-${dayNumber}-detail`} className="scroll-mt-24 py-8 first:pt-0 last:pb-0">
                ...
                {route.summary ? <div className="mt-4 border-l border-white/15 pl-4"><div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.22em] text-white/45"><Navigation className="h-3.5 w-3.5" />当天路线</div><p className="mt-2 max-w-3xl text-sm leading-7 text-white/75">{route.summary}</p></div> : null}
                ...
                {isFirstDay && segment.transport ? <section aria-label={`Day ${dayNumber} 交通`} className="mt-7 border-l border-white/15 pl-4"><p className="text-xs font-semibold uppercase tracking-[0.22em] text-white/45">交通</p><p className="mt-2 max-w-3xl text-sm leading-7 text-white/75">{segment.transport}</p></section> : null}
                <GuideDayStayCard dayNumber={dayNumber} stay={segment.accommodation} staySpot={staysByDay[dayNumber]} note={segment.accommodationNote} continued={!isFirstDay} />
                {isLastDay && segment.practicalTips?.length ? <aside className="mt-6 border-l-2 border-amber-300/40 bg-white/[0.03] px-4 py-3 text-sm leading-7 text-white/68"><p className="font-medium text-amber-100">实用提醒</p><ul className="mt-2 space-y-1">{segment.practicalTips.map((item) => <li key={item}>{item}</li>)}</ul></aside> : null}
