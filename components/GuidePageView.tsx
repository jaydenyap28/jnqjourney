...
import type { PublicGuideTripCost } from '@/lib/guide-budget'
...
export default function GuidePageView({guide,publicData,publicTripCost,approvedPriceHighlights,relatedPackages,allGuides,locale='zh'}:GuidePageViewProps) {
  ...
  const routeRegions = guide.route.map((stop) => {
    ...
    const primaryLabel = locale==='en' ? stopName : regionCn || stopName
  ...
  const selectedSidebarAffiliateIds = guide.sidebarAffiliateLinkIds || guide.featuredAffiliateLinkIds || []
  ...
  const itemListElement = routeRegions.map((stop, index) => ({
    ...
    url: stop.href ? absoluteUrl(localizedPath(stop.href, locale)) : undefined
  }))
  ...
  const structuredData: any = {
    ...
    author: [
      {
        '@type': 'Person',
        name: 'Jayden Yap',
        url: absoluteUrl(localizedPath('/about#jayden', locale)),
      },
      {
        '@type': 'Person',
        name: 'Connie Qing',
        url: absoluteUrl(localizedPath('/about#qing', locale)),
      },
    ],
    publisher: {
      '@type': 'Organization',
    ...
    mainEntityOfPage: {
      '@type': 'WebPage',
      '@id': absoluteUrl(localizedPath(`/guide/${guide.slug}`, locale))
    },
    inLanguage: locale === 'en' ? 'en' : ['zh-CN', 'en'],
  }
  ...
  const breadcrumbData = {
    ...
    itemListElement: [
      ...
      },
      {
        ...
        position: 2,
        name: 'Guides',
        item: absoluteUrl(localizedPath('/guide', locale)),
      },
      {
        '@type': 'ListItem',
        position: 3,
        name: guide.title,
        item: absoluteUrl(localizedPath(`/guide/${guide.slug}`, locale)),
      },
    ],
  }
            ...
            <div className="relative grid gap-7 px-5 py-7 md:px-9 md:py-10 lg:grid-cols-[minmax(0,1.75fr)_minmax(280px,0.85fr)] lg:items-end">
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  <p className="text-[10px] font-semibold uppercase tracking-[0.3em] text-amber-100/82"><PublicCopy text={"Travel Guide / 游记"}/></p>
                ...
                <p className="mt-4 max-w-2xl text-base font-medium leading-7 text-white/90 md:text-xl md:leading-8">
                  {[guide.duration, guide.travelStyle, publicCopy(locale, guide.itineraryMode === 'unassigned' ? '旅行记录' : '完整攻略')].filter(Boolean).join(' · ')}
                </p>
                ...
                <dl className={`mt-6 grid max-w-2xl ${hasPublicTripCost ? 'grid-cols-3' : 'grid-cols-2'} divide-x divide-white/12 border-y border-white/12 bg-black/15 py-3`}>
                  <div className="px-3 first:pl-0">
                    <dt className="text-[10px] uppercase tracking-[0.2em] text-white/45"><PublicCopy text={"行程"}/></dt>
                    <dd className="mt-1 text-sm font-semibold text-white">{guide.duration || '待补充'}</dd>
                  </div>
                  <div className="px-3">
                    <dt className="text-[10px] uppercase tracking-[0.2em] text-white/45"><PublicCopy text={"地区"}/></dt>
                    <dd className="mt-1 text-sm font-semibold text-white">{routeRegions.length}<PublicCopy text={" 个主要地区"}/></dd>
                  </div>
                  {hasPublicTripCost ? (
              ...
              {routeRegions.length ? (
                <section aria-labelledby="hero-route-heading" className="border-l border-white/16 bg-black/20 p-4 backdrop-blur-md md:p-5">
                  <p className="text-[10px] font-semibold uppercase tracking-[0.28em] text-amber-200/75"><PublicCopy text={guide.itineraryMode === 'unassigned' ? 'Destination / 地区' : 'Route / 路线'}/></p>
                  <h2 id="hero-route-heading" className="mt-2 text-xl font-semibold text-white"><PublicCopy text={guide.itineraryMode === 'unassigned' ? '旅行地区' : '路线总览'}/></h2>
                    ...
                    {routeRegions.map((stop, index) => {
                      ...
                      return (
                        <li key={`${stop.name}-${index}`}>
                          <a
                            href={guide.itineraryMode === 'unassigned' ? localizedPath(stop.href || '#itinerary-heading', locale) : `#day-${startDay}`}
                          ...
                          >
                            <span className="font-semibold tabular-nums text-amber-200/80"><PublicCopy text={guide.itineraryMode === 'unassigned' ? '地区' : stop.stopLabel || `D${startDay}`}/></span>
      ...
      <div className="mx-auto max-w-6xl space-y-16 px-4 py-10 md:px-8 md:py-16">
        {guide.itineraryMode !== 'unassigned' && routeMapPoints.length ? (
          <section id="route-map" className="scroll-mt-24">
            <div className="grid gap-3 border-b border-white/10 pb-5 md:grid-cols-[1fr_auto] md:items-end">
              <div>
                <p className="text-[10px] font-semibold uppercase tracking-[0.3em] text-amber-200/68"><PublicCopy text={"Route Map / 路线地图"}/></p>
                <h2 className="mt-2 font-display text-4xl leading-none text-white md:text-5xl"><PublicCopy text={"路线地图"}/></h2>
              </div>
              <p className="max-w-md text-sm leading-6 text-white/55"><PublicCopy text={"点击路线节点查看地区与对应天数；地图不可用时，仍可沿下方文字路线阅读。"}/></p>
            </div>
        ...
        <section aria-labelledby="itinerary-heading" className="min-w-0">
          <div className="border-b border-white/10 pb-5">
            ...
            <h2 id="itinerary-heading" className="mt-2 font-display text-4xl leading-none text-white md:text-5xl"><PublicCopy text={"每日行程"}/></h2>
          </div>
                ...
                <article id={`day-${day.dayNumber}`} key={`${day.date}-${day.dayNumber}`} className="scroll-mt-24 py-10 md:py-14">
                  ...
                  {day.displaySpots.length || day.unresolvedSpotNames.length ? (
                    <section aria-label={`Day ${day.dayNumber} 相关地点`} className="mt-7">
                      <div className="flex items-end justify-between gap-4">
                        <p className="text-xs font-semibold uppercase tracking-[0.22em] text-white/48"><PublicCopy text={"当天相关地点"}/></p>
                        <span className="text-xs tabular-nums text-white/48">{day.displaySpots.length}<PublicCopy text={" 张资料卡"}/></span>
                      </div>
                      ...
                      {day.unresolvedSpotNames.length ? (
                        <div className="mt-3 border-l-2 border-amber-300/45 bg-amber-300/[0.05] px-4 py-3 text-sm leading-6 text-white/68"><PublicCopy text={"\n                          待补完整景点资料："}/>{day.unresolvedSpotNames.join('、')}
                        </div>
                      ) : null}
                    </section>
                  ) : null}
                  ...
                  {day.transport ? (
                    <section aria-label={`Day ${day.dayNumber} 交通`} className="mt-7 border-l border-white/15 pl-4">
                      <p className="text-xs font-semibold uppercase tracking-[0.22em] text-white/45"><PublicCopy text={"交通"}/></p>
                  ...
                  {videoId ? (
                    <section aria-label={`Day ${day.dayNumber} 当日影片`} className="mt-7">
                      <p className="text-xs font-semibold uppercase tracking-[0.22em] text-white/48"><PublicCopy text={"当日影片"}/></p>
        ...
        {hasGuideBookingContent ? (
          <aside id="guide-bookings" aria-labelledby="guide-booking-heading" className="min-w-0 scroll-mt-24 border-y border-white/10 py-6 lg:sticky lg:top-20 lg:border lg:bg-white/[0.025] lg:p-5">
            <div className="mb-5">
              <p className="text-[10px] font-semibold uppercase tracking-[0.28em] text-amber-200/68"><PublicCopy text={"Useful bookings / 实用预订"}/></p>
              <h2 id="guide-booking-heading" className="mt-2 text-2xl font-semibold text-white"><PublicCopy text={"实用预订"}/></h2>
              <p className="mt-2 text-sm leading-6 text-white/56"><PublicCopy text={"以下为这条路线可能用到的交通、住宿或活动预订入口。"}/></p>
            </div>
        ...
        {relatedGuides.length ? (
          <section aria-labelledby="related-guides-heading">
            <div className="border-b border-white/10 pb-5">
              <p className="text-[10px] font-semibold uppercase tracking-[0.3em] text-amber-200/68"><PublicCopy text={"Related Guides / 相关游记路线"}/></p>
              <h2 id="related-guides-heading" className="mt-2 font-display text-4xl leading-none text-white md:text-5xl"><PublicCopy text={"探索更多路线"}/></h2>
            </div>
        ...
        {routeRegions.some((stop) => stop.href) ? (
          <section aria-labelledby="related-regions-heading" className="border-y border-white/10 py-7">
            <h2 id="related-regions-heading" className="text-xl font-semibold text-white"><PublicCopy text={"相关地区与景点"}/></h2>
        ...
        {relatedPackages.length ? (
          <section aria-labelledby="related-packages-heading" className="space-y-5">
            <div>
              <p className="text-[10px] font-semibold uppercase tracking-[0.3em] text-emerald-200/72"><PublicCopy text={"Related travel packages / 相关旅游配套"}/></p>
              <h2 id="related-packages-heading" className="mt-2 font-display text-4xl leading-none text-white"><PublicCopy text={"查看相关旅游配套"}/></h2>
            </div>
        ...
        <section className="grid gap-5 border-t border-white/10 pt-10 lg:grid-cols-[minmax(0,1fr)_320px]">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-semibold text-white"><PublicCopy text={"继续探索 JnQ Journey"}/></h2>
            <div className="mt-5 flex flex-wrap gap-3">
              <Link href="/guide" className="inline-flex min-h-11 items-center gap-2 border border-white/12 px-4 text-sm text-white transition hover:bg-white/[0.06] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-300"><PublicCopy text={"\n                查看更多攻略 "}/><ArrowRight className="h-4 w-4" />
              </Link>
              <Link href="/region" className="inline-flex min-h-11 items-center gap-2 border border-white/12 px-4 text-sm text-white transition hover:bg-white/[0.06] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-300"><PublicCopy text={"\n                按地区找景点 "}/><MapPinned className="h-4 w-4" />
              </Link>
            </div>
          </div>
