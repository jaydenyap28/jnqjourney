      ...
      <div className="mx-auto max-w-5xl px-4 py-8 md:px-8 md:py-12 space-y-12">
        ...
        <header className="space-y-6">
          <div className="space-y-4">
            ...
            <p className="text-lg text-gray-300 leading-relaxed max-w-3xl"><PublicCopy text={"\n              这里整理 JnQ Journey 已收录的 "}/>{readableName}<PublicCopy text={" 景点、美食、路线和旅行参考，适合规划自由行、周末游或亲子行程。\n            "}/></p>
          </div>
        ...
        <section className="space-y-4">
          <h2 className="text-2xl font-bold text-white"><PublicCopy text={"关于 "}/>{readableName}</h2>
        ...
        {relatedPackages.length > 0 ? (
          <section className="space-y-5 rounded-[28px] border border-emerald-200/15 bg-emerald-400/[0.05] p-5 md:p-7">
            <div>
              <p className="text-xs uppercase text-emerald-200/70"><PublicCopy text={"Travel packages / 旅游配套"}/></p>
              <h2 className="mt-2 text-2xl font-bold text-white">{readableName}<PublicCopy text={" 完整旅游配套"}/></h2>
              <p className="mt-2 text-sm leading-7 text-white/60"><PublicCopy text={"想把这里的景点安排进完整行程，可查看已发布配套或通过 WhatsApp 查询。"}/></p>
            </div>
        ...
        {relatedGuides.length > 0 && (
          <section className="space-y-6">
            <h2 className="text-2xl font-bold text-white"><PublicCopy text={"推荐路线"}/></h2>
        ...
        {isCameron && (
          <div className="space-y-12">
            {cameronTea.length > 0 && (
               <section className="space-y-6">
                 <h2 className="text-2xl font-bold text-white"><PublicCopy text={"茶园与咖啡厅"}/></h2>
            ...
            {cameronFarm.length > 0 && (
               <section className="space-y-6">
                 <h2 className="text-2xl font-bold text-white"><PublicCopy text={"亲子农场"}/></h2>
                 <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-4">
            ...
            {cameronGarden.length > 0 && (
               <section className="space-y-6">
                 <h2 className="text-2xl font-bold text-white"><PublicCopy text={"花园打卡"}/></h2>
                 <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-4">
            ...
            {cameronBrinchang.length > 0 && (
               <section className="space-y-6">
                 <h2 className="text-2xl font-bold text-white"><PublicCopy text={"Brinchang 区域"}/></h2>
                 <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-4">
            ...
            {cameronTanahRata.length > 0 && (
               <section className="space-y-6">
                 <h2 className="text-2xl font-bold text-white"><PublicCopy text={"Tanah Rata 区域"}/></h2>
                 <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-4">
            ...
            {cameronKeaFarm.length > 0 && (
               <section className="space-y-6">
                 <h2 className="text-2xl font-bold text-white"><PublicCopy text={"Kea Farm 区域"}/></h2>
                 <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-4">
        ...
        {popularSpots.length > 0 && !isCameron && (
          <section className="space-y-6">
            <h2 className="text-2xl font-bold text-white"><PublicCopy text={"热门景点"}/></h2>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-4">
        ...
        {foodSpots.length > 0 && !isCameron && (
          <section className="space-y-6">
            <h2 className="text-2xl font-bold text-white"><PublicCopy text={"美食与咖啡厅"}/></h2>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-4">
        ...
        {familySpots.length > 0 && !isCameron && (
          <section className="space-y-6">
            <h2 className="text-2xl font-bold text-white"><PublicCopy text={"亲子友好游玩"}/></h2>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-4">
        ...
        {photoSpots.length > 0 && !isCameron && (
          <section className="space-y-6">
            <h2 className="text-2xl font-bold text-white"><PublicCopy text={"绝美拍照打卡"}/></h2>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-4">
        ...
        {locations.length > 0 && (
          <section className="space-y-6 pt-8 border-t border-white/10">
            <h2 className="text-2xl font-bold text-white"><PublicCopy text={"所有收录地点 ("}/>{locations.length})</h2>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-4">
