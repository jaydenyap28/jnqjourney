    ...
    <section id="budget" className="min-w-0 scroll-mt-24">
      <div className="flex flex-col gap-5 border-b border-white/10 pb-5 md:flex-row md:items-end md:justify-between">
        ...
        <fieldset className="min-w-0">
          <legend className="mb-2 text-[11px] font-semibold uppercase tracking-[0.2em] text-amber-100/72">参考币种</legend>
          <div className="grid grid-cols-5 gap-1.5 sm:flex sm:flex-wrap">
            {(['MYR', 'CNY', 'USD', 'JPY', 'SGD'] as GuideDisplayCurrency[]).map((item) => (
              <button key={item} type="button" onClick={() => chooseCurrency(item)} aria-pressed={displayCurrency === item}
                className={`min-h-10 min-w-0 border px-2 text-[11px] font-semibold tracking-[0.06em] transition-colors focus:outline-none focus:ring-2 focus:ring-amber-200/60 sm:min-w-12 sm:px-3 ${displayCurrency === item ? 'border-amber-200/60 bg-amber-300/18 text-amber-50 shadow-[inset_0_-2px_0_rgba(252,211,77,0.65)]' : 'border-white/12 bg-white/[0.025] text-white/55 hover:border-amber-100/35 hover:text-amber-50'}`}>
                {item}
              </button>
            ))}
          </div>
