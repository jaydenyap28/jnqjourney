      ...
      <div className="mx-auto max-w-[1200px] px-5 pb-24 pt-12 md:px-8 md:pt-16">
        <div className="grid grid-cols-2 gap-x-6 gap-y-11 lg:grid-cols-[1.3fr_0.75fr_0.85fr_1.1fr] lg:gap-12">
          <section className="col-span-2 lg:col-span-1">
            ...
            <p className="font-cjk-display mt-2 text-xl text-white/88"><PublicCopy text={"一起看世界"}/></p>
            <p className="mt-4 max-w-sm text-sm leading-7 text-white/55"><PublicCopy text={"\n              旅游地图、景点资料、路线攻略与旅行记录，由 JnQ Journey 持续整理。\n            "}/></p>
          ...
          <nav aria-label="探索">
            <h2 className="text-sm font-semibold text-white"><PublicCopy text={"探索"}/></h2>
            <ul className="mt-5 space-y-3 text-sm text-white/55">
              {exploreLinks.map((item) => (
                <li key={item.href}>
                  <Link href={item.href} className="inline-flex min-h-7 items-center transition hover:text-white"><PublicCopy text={item.label}/></Link>
                </li>
              ))}
            </ul>
          </nav>
          ...
          <nav aria-label="网站说明">
            <h2 className="text-sm font-semibold text-white"><PublicCopy text={"网站说明"}/></h2>
            <ul className="mt-5 space-y-3 text-sm text-white/55">
          ...
          <section className="col-span-2 border-t border-white/10 pt-7 lg:col-span-1 lg:border-t-0 lg:pt-0">
            <h2 className="text-sm font-semibold text-white"><PublicCopy text={"联系 Jayden &amp; Qing"}/></h2>
            <p className="mt-5 text-sm leading-7 text-white/55"><PublicCopy text={"\n              旅游配套、报价及商业合作，请从联系页面选择合适的查询类型。\n            "}/></p>
            <Link href="/contact" className="mt-5 inline-flex min-h-10 items-center text-sm font-medium text-white/75 transition hover:text-white"><PublicCopy text={"\n              联系我们\n            "}/></Link>
          </section>
        </div>

        <div className="mt-12 border-t border-white/10 pt-6">
          <p className="max-w-4xl text-xs leading-6 text-white/38"><PublicCopy text={"\n            部分预订入口可能是联盟链接；符合条件的交易可能为本站带来佣金，通常不会增加读者费用。价格、库存与条款以相关供应方最终确认结果为准。\n          "}/></p>
