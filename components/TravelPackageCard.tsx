  ...
  return (
    <article className="overflow-hidden rounded-[24px] border border-white/10 bg-[linear-gradient(180deg,rgba(18,31,50,0.96),rgba(8,15,28,0.98))]">
      <div className={`relative ${compact ? 'aspect-[16/8]' : 'aspect-[16/9]'} bg-[#0d1726]`}>
        {item.cover_image ? <FallbackImage src={item.cover_image} alt={item.title_zh} fill className="object-cover" /> : <div className="flex h-full items-center justify-center text-white/20"><ImageOff className="h-8 w-8" aria-hidden="true" /></div>}
      </div>
