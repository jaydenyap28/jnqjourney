...
import { canonicalGuideBudgetItems, divideSnapshotMoney, formatGuideBudgetCents, guideBudgetMoneyToCents, resolvePublicGuideTripCost, type GuideBudgetDisplaySnapshot } from '@/lib/guide-budget'
...
export default function GuideBudgetSection({ guide, actualSpend, showAdminSourceNote = false }: { guide: Pick<TravelGuide, 'budget' | 'budgetItems' | 'budgetScope'>; actualSpend: TripCostSnapshot | null; showAdminSourceNote?: boolean }) {
  const tripCost = resolvePublicGuideTripCost(actualSpend, guide.budgetItems)
  if (tripCost.source === 'hidden') return null
  const categories = tripCost.categories
  const totalCents = tripCost.totalCents
