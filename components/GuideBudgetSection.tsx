...
import { divideSnapshotMoney, formatSnapshotMoney, getTripCostCategory, orderedTripCostCategoryEntries, type GuideBudgetDisplaySnapshot } from '@/lib/guide-budget'

type TripCostSnapshot = Omit<GuideBudgetDisplaySnapshot, 'source_project_name'> & { source_project_name?: string }
const ICONS = { Plane, TrainFront, BedDouble, Utensils, Ticket, Sparkles, Wifi, ShoppingBag, CircleEllipsis }
...
function formatEstimatedMoney(value?: string | null, fallbackCurrency = 'RM') {
  ...
  if (!text) return ''
  return /^(rm|cny|jpy|thb|idr|usd|rmb|myr)/i.test(text) ? text : `${fallbackCurrency} ${text}`
}
...
function CategoryGrid({ categories, currency, total, tone }: { categories: Array<{ key: string; label: string; amount: number; note?: string }>; currency: string; total: number; tone: 'estimated' | 'actual' }) {
  return <div className="grid min-w-0 grid-cols-1 gap-3 min-[360px]:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
    {categories.map((item) => {
      const category = getTripCostCategory(item.key)
      const Icon = ICONS[category?.icon as keyof typeof ICONS] || CircleEllipsis
      ...
      return <article key={item.key} className="min-w-0 border border-white/10 bg-[#0b111d] px-4 py-4 md:px-5">
        ...
        <p className="mt-2 break-words text-lg font-semibold leading-tight tabular-nums text-white md:text-xl">{formatSnapshotMoney(currency, item.amount)}</p>
...
export default function GuideBudgetSection({ guide, actualSpend, showAdminSourceNote = false }: { guide: Pick<TravelGuide, 'budget' | 'budgetItems' | 'budgetScope'>; actualSpend: TripCostSnapshot | null; showAdminSourceNote?: boolean }) {
  const estimatedItems = guide.budgetItems.map((item) => {
    const category = getTripCostCategory(item.label)
    return { key: category?.key || item.label, label: category ? `${category.nameEn} / ${category.nameZh}` : item.label || '预算项目', amount: parseMoney(item.amount), note: item.note }
  ...
  const actualCategories = actualSpend ? orderedTripCostCategoryEntries(Object.entries(actualSpend.categories).map(([key, value]) => {
    const category = getTripCostCategory(key)
    return { key: category?.key || key, label: category ? `${category.nameEn} / ${category.nameZh}` : key, amount: Number(value) }
  }).filter((item) => Number.isFinite(item.amount) && item.amount > 0)) : []
  if (!actualSpend && !(declaredEstimatedTotal > 0 || estimatedItems.length)) return null
  const categories = actualSpend ? actualCategories : orderedTripCostCategoryEntries(estimatedItems)
  const total = actualSpend ? Number(actualSpend.total) : estimatedTotal
  ...
  return <section id="budget" className="scroll-mt-24"><div className="border-b border-white/10 pb-5"><h2 className="font-display text-4xl leading-none text-white md:text-5xl">Trip Cost <span className="text-white/42">/ 旅程花费</span></h2></div><div className="mt-6"><div className={`border p-5 md:p-6 ${actualSpend ? 'border-emerald-200/16 bg-[radial-gradient(circle_at_top_left,rgba(52,211,153,0.13),transparent_38%),#0b111d]' : 'border-amber-200/14 bg-[radial-gradient(circle_at_top_left,rgba(245,158,11,0.13),transparent_38%),#0b111d]'}`}><span className={`inline-flex rounded-full border px-3 py-1 text-[10px] font-semibold tracking-[0.16em] ${actualSpend ? 'border-emerald-200/20 bg-emerald-300/[0.08] text-emerald-100/80' : 'border-amber-200/20 bg-amber-300/[0.08] text-amber-100/80'}`}>{actualSpend ? '实际花费' : '预算'}</span><div className="mt-5 grid min-w-0 gap-6 lg:grid-cols-[minmax(0,1.15fr)_minmax(0,1.85fr)] lg:items-end"><div className="min-w-0"><div className={actualSpend ? 'text-emerald-100/75' : 'text-amber-100/75'}><div className="flex items-center gap-2"><Wallet className="h-4 w-4" /><p className="text-xs font-semibold uppercase tracking-[0.22em]">{actualSpend ? '实际总支出' : estimatedScopeLabel(guide.budgetScope)}</p></div></div><p className="mt-3 break-words text-[clamp(2rem,5vw,3.7rem)] font-semibold leading-none tabular-nums text-white">{actualSpend ? formatSnapshotMoney(currency, actualSpend.total) : declaredEstimatedTotal > 0 ? formatEstimatedMoney(guide.budget) : formatSnapshotMoney(currency, total)}</p></div>{actualSpend ? <dl className="grid min-w-0 grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">{actualSpend.traveller_count && actualSpend.scope !== 'unspecified' ? <SummaryMetric icon={<Users className="h-3.5 w-3.5" />} label="旅客人数" value={`${actualSpend.traveller_count} 位旅客`} /> : null}{average ? <SummaryMetric icon={<Calculator className="h-3.5 w-3.5" />} label="平均每人" value={`${formatSnapshotMoney(currency, average)}／人`} /> : null}<SummaryMetric icon={<ReceiptText className="h-3.5 w-3.5" />} label="记录数量" value={`${actualSpend.transaction_count} 笔`} /><SummaryMetric icon={<RefreshCw className="h-3.5 w-3.5" />} label="最后同步" value={formatDate(actualSpend.received_at)} /></dl> : <p className="max-w-lg border-t border-white/10 pt-4 text-sm leading-6 text-white/48 lg:border-l lg:border-t-0 lg:pl-6 lg:pt-0">金额会因人数、汇率与消费习惯不同而变化</p>}</div>{showAdminSourceNote && actualSpend?.source_project_name ? <p className="mt-5 border-t border-white/8 pt-4 text-xs text-white/45">实际花费根据 MoneyBot「{actualSpend.source_project_name}」项目记账整理。</p> : null}</div>{categories.length ? <div className="mt-3"><CategoryGrid categories={categories} currency={currency} total={total} tone={actualSpend ? 'actual' : 'estimated'} /></div> : null}</div></section>
}
