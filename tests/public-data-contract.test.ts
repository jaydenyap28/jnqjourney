...
test('Spot API and admin refresh preserve cache contracts', async () => {
  ...
  assert.match(adminRoute, /resolvePublicSnapshotBundleUncached\(\)/)
  assert.match(adminRoute, /uploadPublicDataSnapshot\('locations\.json'/)
  assert.doesNotMatch(adminRoute, /resolvePublicData\(\)/)
  assert.match(adminRoute, /revalidateTag\('public-data'\)/)
  assert.match(adminRoute, /revalidateTag\('public-locations'\)/)
  assert.match(adminRoute, /revalidateTag\('public-spots'\)/)
  assert.match(adminRoute, /revalidateTag\(`public-spot:\$\{spot\.slug\}`\)/)
  assert.match(page, /dataSource=\{resolved\.source\}/)
  assert.match(await read('components/SpotPageView.tsx'), /data-jnq-data-source=\{dataSource\}/)
