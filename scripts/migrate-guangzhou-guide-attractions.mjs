...
import { GUANGZHOU_SLUG, planGuangzhouMigration, validateGuangzhouCanonical } from '../lib/guangzhou-guide-migration.ts'
...
if (resumeArg) {
  ...
  if (!directory.startsWith(path.resolve('artifacts/guangzhou-migration') + path.sep)) throw new Error('Resume must use an existing migration backup directory')
...
const matches = guides.filter(g => g.slug === GUANGZHOU_SLUG)
if (matches.length !== 1) throw new Error('Expected unique Guangzhou Guide')
const before = matches[0]
const locations = await sb.from('locations').select('id,name,name_cn,status').order('id').range(0, 999)
if (locations.error) throw new Error(locations.error.message)
const plan = planGuangzhouMigration(before, locations.data)
const boundIds = new Set(plan.after.days.flatMap(day => day.attractions.map(ref => ref.spotId)))
if (locations.data.filter(spot => boundIds.has(spot.id)).some(spot => spot.status !== 'active')) throw new Error('Bound Spot is inactive; re-audit required')
const nextGuides = guides.map(g => g.slug === GUANGZHOU_SLUG ? plan.after : g)
const unchangedGuides = guides.filter(g => g.slug !== GUANGZHOU_SLUG).map(g => ({ slug: g.slug, before: hash(g), after: hash(nextGuides.filter(n => n.slug === g.slug)[0]) }))
...
const outDir = `artifacts/guangzhou-migration/${Date.now()}`
...
validateGuangzhouCanonical(publishedGuides.filter(g => g.slug === GUANGZHOU_SLUG)[0])
