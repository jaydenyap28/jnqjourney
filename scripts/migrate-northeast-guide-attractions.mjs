...
import { NORTHEAST_SLUG, planNortheastMigration, validateNortheastCanonical } from '../lib/northeast-guide-migration.ts'
...
if (resumeArg) {
  ...
  if (!directory.startsWith(path.resolve('artifacts/northeast-migration') + path.sep)) throw new Error('Resume must use an existing migration backup directory')
...
const matches = guides.filter(g => g.slug === NORTHEAST_SLUG)
if (matches.length !== 1) throw new Error('Expected unique Northeast Guide')
...
const plan = planNortheastMigration(before, locations.data)
...
const nextGuides = guides.map(g => g.slug === NORTHEAST_SLUG ? plan.after : g)
const unchangedGuides = guides.filter(g => g.slug !== NORTHEAST_SLUG).map(g => ({ slug: g.slug, before: hash(g), after: hash(nextGuides.filter(n => n.slug === g.slug)[0]) }))
...
const outDir = `artifacts/northeast-migration/${Date.now()}`
...
validateNortheastCanonical(publishedGuides.filter(g => g.slug === NORTHEAST_SLUG)[0])
