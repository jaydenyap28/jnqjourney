...
import { YUNNAN_SLUG, planYunnanMigration, validateYunnanCanonical } from '../lib/yunnan-guide-migration.ts'
...
if (resumeArg) {
  ...
  if (!directory.startsWith(path.resolve('artifacts/yunnan-migration') + path.sep)) throw new Error('Resume must use an existing migration backup directory')
...
const matches = guides.filter(g => g.slug === YUNNAN_SLUG)
if (matches.length !== 1) throw new Error('Expected unique Yunnan Guide')
...
const plan = planYunnanMigration(before, locations.data)
...
const nextGuides = guides.map(g => g.slug === YUNNAN_SLUG ? plan.after : g)
const unchangedGuides = guides.filter(g => g.slug !== YUNNAN_SLUG).map(g => ({ slug: g.slug, before: hash(g), after: hash(nextGuides.filter(n => n.slug === g.slug)[0]) }))
...
const outDir = `artifacts/yunnan-migration/${Date.now()}`
...
validateYunnanCanonical(publishedGuides.filter(g => g.slug === YUNNAN_SLUG)[0])
