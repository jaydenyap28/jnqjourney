...
export function createLocalizationIO() {
  ...
  return {
    ...
    async lock() {
      const acquire = () => bucket.upload(
        lockKey,
        Buffer.from(JSON.stringify({startedAt:new Date().toISOString()})),
        {upsert:false,contentType:'image/webp',cacheControl:'0'}
      )

      let { error } = await acquire()
      if (!error) return

      // Vercel writers finish within a minute. If a prior invocation was interrupted,
      // self-heal a lock that has clearly outlived any legitimate publication.
      const { data: existing } = await bucket.download(lockKey)
      if (existing) {
        try {
          const payload = JSON.parse(Buffer.from(await existing.arrayBuffer()).toString())
          const startedAt = Date.parse(String(payload?.startedAt || ''))
          const stale = Number.isFinite(startedAt) && Date.now() - startedAt > 10 * 60 * 1000
          if (stale) {
            const { error: removeError } = await bucket.remove([lockKey])
            if (!removeError) {
              ;({ error } = await acquire())
              if (!error) return
            }
          ...
          // Keep the conservative locked behavior if the lock cannot be verified as stale.
        }
      }

      throw Error('Localization publication is locked. Another editor may be publishing; a lock left by an interrupted process requires operator review.')
    },
    async unlock() {
