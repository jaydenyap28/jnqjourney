...
import { assertCurrentSpotGenerationRequest, parseSpotEnglishGeneration, spotEnglishGenerationSchema, spotTranslationFieldLimit } from '@/lib/spot-localization-generation'
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const generated = parseSpotEnglishGeneration(
      await generateGeminiJson({
        instructions: translationInstructions,
        input: JSON.stringify(source),
        schema: spotEnglishGenerationSchema as unknown as Record<string, unknown>,
      }),
      source
    )
